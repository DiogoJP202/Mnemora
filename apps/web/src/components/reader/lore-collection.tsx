...
export function LoreCollection({ bookId, config }: { bookId: string; config: CollectionConfig }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiGet<LoreEntitySummary[]>(`/api/books/${bookId}${config.endpoint}`),
    ]).then(([foundBook, library, knownEntities]) => {
      if (!active) return;
      setBook(foundBook);
      setEntities(knownEntities);
  ...
  return <div className="reader-page lore-page">
    ...
    <BookLoreNav bookId={bookId} active={config.segment} />
