...
export function TimelineView({ bookId }: { bookId: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const knownEvents = await apiGet<TimelineEvent[]>(`/api/books/${bookId}/timeline`);
      if (active) setEvents(knownEvents);
    }).catch((cause) => {
  ...
  return <div className="reader-page lore-page timeline-page">
    ...
    <BookLoreNav bookId={bookId} active="/timeline" memoryPackAvailable={book?.memoryPackAvailable ?? false} />
