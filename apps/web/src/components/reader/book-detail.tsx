...
export function BookDetail({ bookId }: { bookId: string }) {
  ...
  return (
    <div className="reader-page book-detail-page">
      ...
      <div className="book-hero"><BookCover title={book.title} coverUrl={book.coverUrl} large /><div><span className="section-index">SEU LIVRO</span><h1>{book.title}</h1><p className="book-author">por {book.author}</p>{libraryItem ? <div className="current-progress"><span>SEU PROGRESSO</span><strong>{libraryItem.currentReadingUnitId ? `${Math.round(progress)}%` : "Ainda não começou"}</strong><div className="book-progress-track" role="progressbar" aria-label={`Progresso em ${book.title}`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(progress)}><i aria-hidden="true" style={{ width: `${progress}%` }} /></div></div> : <button className="button button-ink" onClick={addToLibrary} disabled={saving} type="button">{saving ? "Adicionando…" : "Adicionar à minha biblioteca"} <span aria-hidden="true">＋</span></button>}</div></div>
