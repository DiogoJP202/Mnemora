...
export function LibraryView() {
  ...
  return (
    <div className="reader-page" aria-busy={!books && !error}>
      ...
      {!!books?.length && <><div className="library-count"><span>{books.length === 1 ? "1 LIVRO NA SUA ESTANTE" : `${books.length} LIVROS NA SUA ESTANTE`}</span><span className="library-rule" /></div><div className="card-grid">{books.map((item) => { const progress = Math.max(0, Math.min(100, item.progressPercentage ?? 0)); return <Link href={`/app/books/${item.book.id}`} className="book-card" key={item.book.id}><BookCover title={item.book.title} coverUrl={item.book.coverUrl} /><div className="book-card-info"><span className="book-status">{statusLabel[item.status] ?? item.status}</span><h2>{item.book.title}</h2><p>{item.book.author}</p><div className="book-progress"><div className="book-progress-track" role="progressbar" aria-label={`Progresso em ${item.book.title}`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(progress)}><i aria-hidden="true" style={{ width: `${progress}%` }} /></div><span>{item.currentReadingUnitId ? `${Math.round(progress)}% da leitura` : "Ainda não começou"}</span></div></div><span className="book-card-arrow" aria-hidden="true">↗</span></Link>; })}</div></>}
    </div>
  );
}
