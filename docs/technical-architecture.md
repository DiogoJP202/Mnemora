...
flowchart LR
    ...
    Infra -. metadados opcionais .-> Google[Google Books]
```
...
Não há cache distribuído, fila ou serviço de IA. O estado persistente fica no SQLite. O Google Books é um provedor opcional e serve somente para metadados bibliográficos.
...
| `src/Mnemora.Infrastructure` | EF Core, Identity, SQLite, consultas seguras, consistência, seed e Google Books | Application e Domain |
...
- `Book` guarda catálogo, autoria, capa, ISBN e identificadores externos. A combinação de provedor e identificador externo é única.
...
Um middleware aplica `Cache-Control: no-store` a toda resposta sob `/api`. O cliente também usa `fetch` com `cache: "no-store"` e `credentials: "same-origin"`. CORS permite apenas a origem configurada em `Mnemora:WebOrigin`. A busca externa tem timeout e importa título, autor, capa e dados bibliográficos; a descrição do provedor é descartada por poder conter spoilers. Cada usuário pode manter até 200 notas por livro, e a revisão conserva as 500 atividades mais recentes por usuário e livro. Essas escritas são serializadas no processo, de acordo com a implantação SQLite de instância única prevista para o MVP.
...
2. A busca consulta o catálogo local e, se houver `GOOGLE_BOOKS_API_KEY`, o Google Books.
3. A seleção adiciona o livro à biblioteca de forma idempotente.
4. O leitor escolhe a unidade alcançada; a página, quando informada, não muda o conhecimento.
5. Personagens, facções, lugares, timeline, detalhe e Recall são projetados para a ordem atual.
6. Notas gerais ficam disponíveis ao dono; notas vinculadas acompanham a visibilidade da referência.
7. A revisão seleciona deterministicamente até cinco entidades conhecidas e registra o feedback.
...
- Livros importados podem não ter unidades ou pack de lore; nesse caso, a interface mantém estados vazios e não tenta deduzir conteúdo.
